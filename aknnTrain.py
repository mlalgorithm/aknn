...
def lbsmodel(X,Y,rmax,outfile,ncpu,dmean,dstd,dmin,dmax):
    itermax=50
    ...
    for k in np.arange(2,itermax):
        ...
        if smaxk<loss:
            ...
            if k==itermax-1:
                ...
                np.savez(outfile,loss=loss,subset=subset,dmean=dmean,dstd=dstd,dmin=dmin,dmax=dmax)
                return loss
        else:
...
def main():
    ...
    #print(np.where(np.sum(data,axis=0)>=0.0001)[0])
    #jstd=np.where(np.std(data,axis=0)>=0.0001)[0]
    #print(jstd)
    #data=data[:,jstd]
    ...
    if args.out[-4:]!='.npz':
        ...
        print(args.out)
    lbsmodel(data[:,:-1],label,args.ne,args.out,args.cpu,dmean,dstd,dmin,dmax)
